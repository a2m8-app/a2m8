...
use mlua::{Lua, UserDataMethods, UserData};
use std::io::Read;
...
pub async fn run_command(_: &Lua, (command, cwd): (String, Option<String>)) -> mlua::Result<CommandResult> {
    ...
    let (mut stdout, mut stdout_writer) = os_pipe::pipe()?;
    let (mut stderr, mut stderr_writer) = os_pipe::pipe()?;
    let stdin = ShellPipeReader::stdin();

    let exit_code = deno_task_shell::execute_with_pipes(list, env_from_proc, &cwd, stdin,
        ...
        let stdout = String::from_utf8(stdout_buffer).map_err(|x| 
            mlua::Error::RuntimeError(format!("Failed to convert stdout to utf8: {}", x))
        )?;
        let stderr = String::from_utf8(stderr_buffer).map_err(|x| 
            mlua::Error::RuntimeError(format!("Failed to convert stderr to utf8: {}", x))
        )?;
    Ok(CommandResult {
        stdout: stdout,
        stderr: stderr,
...
pub async fn run_command_piped(_: &Lua, (command, cwd): (String, Option<String>)) -> mlua::Result<i32> {
    ...
    let exit_code = deno_task_shell::execute(list, env_from_proc, &cwd
        ).await;
        Ok(exit_code)
...
impl UserData for CommandResult {
    fn add_fields<'lua, F: mlua::UserDataFields<'lua, Self>>(fields: &mut F) {
        fields.add_field_method_get("stdout", |_, this: &CommandResult| {
            Ok(this.stdout.clone())
        });
        fields.add_field_method_get("stderr", |_, this: &CommandResult| {
            Ok(this.stderr.clone())
        });
        fields.add_field_method_get("exit_code", |_, this: &CommandResult| {
            Ok(this.exit_code)
        });
    }
}